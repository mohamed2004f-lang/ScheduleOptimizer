...
QUESTION_SEED: dict[str, list[tuple[str, int]]] = {
    ...
    "alumni": [
        ("المحتوى الذي درسته كان متماسكاً ومفيداً", 10),
        ("عند التخرج كنت قادراً على أداء مهام مهنتي الأساسية", 20),
        ("المقررات ربطتني بمتطلبات سوق العمل في تخصصي", 30),
        ("طوّرت مهارات التواصل والعمل ضمن فريق", 40),
        ("ساعدتني الكلية في التدريب أو التوجيه المهني", 50),
        ("بشكل عام، أنصح بدراسة هذا التخصص في الكلية", 60),
    ],
    "staff_student_services": [
